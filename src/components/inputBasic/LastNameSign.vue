...
export default {
  ...
  methods: {
    ...
    blurFn () {
      ...
      this.$emit('endStateResLastNameSign', this.endState)
    },
    changeMessage (event) {
      ...
      if (this.message.length > 51) {
        this.errChk = true
        this.effChk = false
        this.endState = false
        this.errMsg = '50글자 이내로 입력해주세요.'
      }
      if (this.message.length >= 1 && this.message.length <= 50) {
        this.errChk = true
        this.effChk = true
        this.endState = true
        this.errMsg = 'LAST NAME 입력이 완료되었습니다.'
      }
      this.$emit('input', this.message)
      this.$emit('chgLastNameSignRes', this.message)
      this.$emit('endStateResLastNameSign', this.endState)
    ...
    initMessage (msg) {
      ...
      if (this.message.length > 0) {
        ...
        if (this.message.length >= 1 && this.message.length <= 50) {
          ...
          this.effChk = true
          this.endState = true
          this.errMsg = 'FIRST NAME 입력이 완료되었습니다.'
        }
        if (this.message.length === 0) {
          this.focusState = false
          this.effChk = false
