...
export default {
  ...
  methods: {
    ...
    blurFn () {
      ...
      this.$emit('endStateResPassLogin', this.endState)
    },
    changeMessage (event) {
      ...
      if (this.message.length >= 6 && this.message.length <= 50) {
        this.errChk = true
        this.effChk = true
        this.endState = true
        this.errMsg = '비밀번호 입력이 완료되었습니다.'
      }
      this.$emit('input', this.message)
      this.$emit('chgPassLoginRes', this.message)
      this.$emit('endStateResPassLogin', this.endState)
